...
**Examples:**
- Changes in `aspire1.ApiService/Services/CachedWeatherService.cs` + `aspire1.Web/Program.cs` → `(api,web)`
- Changes in `aspire1.ApiService.Tests/Services/CachedWeatherServiceTests.cs` → `(api,test)` or just `(test)` if only test files
...
📦 Staged files: 3
   - aspire1.ApiService/Services/CachedWeatherService.cs
   - aspire1.Web/Program.cs
   - aspire1.ApiService.Tests/Services/CachedWeatherServiceTests.cs
...
🚀 Pushing to origin/feature/redis-caching...
✅ Pushed successfully!
